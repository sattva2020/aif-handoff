...
| `AIF_USAGE_LIMITS_ENABLED`         | boolean | `true`                         | Enable live refresh of Codex / Claude runtime-limit snapshots. When `false`, `/runtime-profiles` skips the filesystem scan of `~/.codex/sessions` and the Claude provider-identity lookup; persisted snapshots are still returned but never updated. Useful on machines with thousands of Codex rollouts where cold refreshes are expensive                                                                                                                                                                     |
