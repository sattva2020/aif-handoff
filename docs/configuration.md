...
| `AGENT_USE_SUBAGENTS`              | boolean | `true`              | Stage execution mode. `true`: run custom agents (`plan-coordinator`, `implement-coordinator`, sidecars). `false`: use `aif-plan`, `aif-implement`, `aif-review`, `aif-security-checklist` directly |
